...
async def chat_endpoint(
    ...
    if not is_scam_confirmed:
        ...
        if detection_data:
            ...
            if is_scam:
                ...
                # Early exit for non-scam
                return AgentResponse(
                    status="success",
                    reply="Sorry, I think you have the wrong number."
                )
        else:
             # Fallback if detector fails completely
             return AgentResponse(
                status="success",
                reply="Hello? Who is this?"
            )
    ...
    try:
        ...
        logger.error(f"Persona Generation Failed: {e}")
        agent_reply_text = "hello? i cannot hear you properly.. message again?"

    # 5. Metrics & Termination Logic
    total_messages = len(payload.conversationHistory) + 2
    
    # Check for meaningful data
    valid_bank_accounts = [acc for acc in current_intel.bankAccounts if len(re.sub(r"\D", "", acc)) >= 9]
    valid_phone_numbers = [phone for phone in current_intel.phoneNumbers if len(re.sub(r"\D", "", phone)) >= 10]
    ...
    is_long_convo = total_messages > 20
    min_accumulation_depth = 8 # Give it at least 4 turns to try and get intel
    
    should_report = is_scam_confirmed and (
        (has_critical_intel and total_messages >= min_accumulation_depth) 
        or is_long_convo
    )
